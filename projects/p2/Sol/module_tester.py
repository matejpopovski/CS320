...
def loans_test():
    ...
    assert loans.Applicant("25-34", []).lower_age() == 25
    assert loans.Applicant("35-44", []).lower_age() == 35
    assert loans.Applicant("45-54", []).lower_age() == 45
    assert loans.Applicant("55-64", []).lower_age() == 55
    assert loans.Applicant("65-74", []).lower_age() == 65
    loans_points += 4
